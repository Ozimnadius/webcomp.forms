 ...
 *
 * Сервис валидирует поля (обязательность, допустимые варианты, предельную
 * длину), сохраняет результат как элемент инфоблока формы и ставит в очередь
 * почтовое событие. Сбой почты после успешного сохранения не считается ошибкой
 * отправки: заявка уже в базе, ошибка пишется в журнал. Компонент передает
 * сюда уже нормализованные поля из FormRepository и данные запроса.
 *
...
class ResultService
{
     ...
     *
     * Проверяются обязательные поля, для select/radio/checkbox с OPTIONS -
     * принадлежность отправленного значения списку допустимых вариантов,
     * для всех полей - предельная длина значения. Более специфичная ошибка
     * (обязательность, недопустимый вариант) имеет приоритет над ошибкой длины.
     *
     * @param array<int, array<string, mixed>> $fields Нормализованные поля формы.
     * @param array<string, mixed> $requestData Данные POST-запроса.
     *
     ...
     *
     * Возвращает HTML-безопасную строку: пользовательский ввод экранируется,
     * переводы строк превращаются в <br />, технические значения списочных
     * полей заменяются на тексты вариантов. Шаблон письма имеет BODY_TYPE=html,
     * поэтому неэкранированное значение означало бы HTML-инъекцию в письмо.
     *
     * @param array<string, mixed> $field Поле формы.
     * @param mixed $value Значение из запроса.
     *
